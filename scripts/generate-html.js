        ...
        <div class="file-operations">
          ...
          <div class="file-menu-wrapper">
            ...
            <div class="file-menu-popover" hidden>
              ...
              <button class="file-menu-item" data-action="overwrite-draft">Overwrite Draft</button>
              <button class="file-menu-item" data-action="switch-to-draft">Switch to Draft</button>
              <button class="file-menu-item" data-action="save-as-new">Save as New...</button>
              <button class="file-menu-item" data-action="revert-to-published">Revert to Published</button>
              <div class="file-menu-divider"></div>
...
function renderFormulaBar(config) {
  return `      <!-- ============================================ -->
      ...
      <div class="formula-bar">
        ...
        <div contenteditable="plaintext-only" class="formula-input" role="textbox" aria-label="Cell value or formula" data-placeholder="Enter formula or value" spellcheck="false" autocorrect="off" autocapitalize="off"></div>
      </div>`;
}
...
// Grid expansion controls (add-columns / add-rows buttons) are now rendered
// inline inside the grid scroll container by renderGrid(), so they scroll with
...
function generateGridHeader(gridConfig) {
  ...
  if (hasStopColumn) {
    headerCells += '\n              <th class="grid-add-col-header"><button class="add-columns-btn" title="Add column">+</button></th>';
...
function generateGridRow(rowNum, gridConfig, isFirstRow) {
  ...
  if (hasStopColumn) {
    // Only first row gets the add-col and separator columns with rowspan
    if (isFirstRow) {
      cells += '\n              <td class="grid-add-col-cell" rowspan="10000"></td>';
...
<!--
  ...
  ============================================================================
  This file is generated by: scripts/generate-html.js
