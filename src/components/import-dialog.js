...
export function createImportDialog() {
  ...
  let importing = false;
  let existingPackageMatch = null; // { folderId, folderName, exportedAt } when packageId matches an existing import
  let replaceExisting = true;       // user choice: replace existing copy on packageId match
  ...
  let checkMissingScenarioFunctions = null;
  ...
  function init(deps) {
    ...
    checkMissingScenarioFunctions = deps.checkMissingScenarioFunctions;
  ...
  async function showWithImportData(importData, sourceName) {
    ...
    // Check for an existing copy with the same packageId — supports re-importing
    // an updated zip without piling up duplicate folders.
    const packageId = importData.manifest?.packageId || null;
    if (packageId && storageEngine.findFolderByPackageId) {
      ...
      if (existing) {
        existingPackageMatch = {
          ...
          exportedAt: existing.folder.exportedAt || null,
        };
      }
    }
    ...
    // Check for missing dependencies (sheet-level + scenario-level)
    dependencyWarnings = await checkMissingDependencies(currentImportData, storageEngine);
    if (checkMissingScenarioFunctions) {
      scenarioWarnings = await checkMissingScenarioFunctions(currentImportData, storageEngine);
    }
  ...
  async function handleImport() {
    ...
    try {
      ...
      // If user opted to replace an existing copy of the same package, delete it
      // before importing — the new import then takes the slot in Downloads.
      if (existingPackageMatch && replaceExisting && storageEngine.deleteFolder) {
        try {
          await storageEngine.deleteFolder(existingPackageMatch.folderId);
        } catch (err) {
          console.warn('[ImportDialog] Failed to delete existing copy:', err);
          // Non-fatal — proceed with import as new copy
        }
      }

      const packageId = currentImportData.manifest?.packageId || null;
  ...
  function render() {
    ...
    const scenarioCount = Object.keys(currentImportData.scenarios || {}).length;

    const corruptScenarios = currentImportData.corruptScenarios || [];
    ...
    bodyEl.innerHTML = `
      ...
      <!-- Existing-package banner (packageId match) -->
      ${existingPackageMatch ? `
        <div class="import-existing-package">
          <label>
            <input type="checkbox" class="import-replace-existing-checkbox" ${replaceExisting ? 'checked' : ''} />
            Replace existing copy <strong>"${escapeHtml(existingPackageMatch.folderName)}"</strong>${existingPackageMatch.exportedAt ? ` (loaded ${escapeHtml(existingPackageMatch.exportedAt.split('T')[0])})` : ''}
          </label>
          <p class="import-replace-existing-hint">${replaceExisting ? 'The existing copy will be deleted before this import.' : 'This package will be added as a new copy alongside the existing one.'}</p>
        </div>
      ` : ''}
      ...
      <div class="import-summary">
        ...
        <ul>
          ...
          ${scenarioCount > 0 ? `<li>${scenarioCount} scenario analys${scenarioCount !== 1 ? 'es' : 'is'}</li>` : ''}
        </ul>
      </div>
      ...
      <!-- Dependency Warnings -->
      ${(dependencyWarnings.length > 0 || scenarioWarnings.length > 0 || corruptScenarios.length > 0) ? renderWarnings(corruptScenarios) : ''}
    `;
    ...
    const replaceCheckbox = bodyEl.querySelector('.import-replace-existing-checkbox');
    if (replaceCheckbox) {
  ...
  function renderWarnings(corruptScenarios = []) {
    const reasonLabel = (reason) => {
      if (reason === 'missing-file') return 'data file missing from package';
      if (reason === 'parse-error') return 'data file is corrupted';
    ...
    return `
      <div class="import-warnings">
        ...
        ${scenarioWarnings.length > 0 ? `
          <p>The following scenario analyses target functions that are not available locally or in this package:</p>
          <ul>
            ${scenarioWarnings.map(w => `
              <li>
                <strong>${escapeHtml(w.scenarioName)}</strong>:
                ${escapeHtml(w.functionName)}
              </li>
            `).join('')}
          </ul>
        ` : ''}
        ${corruptScenarios.length > 0 ? `
          <p>The following scenario analyses could not be loaded and will be skipped:</p>
          <ul>
            ${corruptScenarios.map(c => `
              <li>
                <strong>${escapeHtml(c.scenarioName)}</strong>:
                ${escapeHtml(reasonLabel(c.reason))}
              </li>
            `).join('')}
          </ul>
        ` : ''}
        <p class="import-warning-note">These items may not work correctly until the missing functions are imported.</p>
      </div>
    `;
  }
