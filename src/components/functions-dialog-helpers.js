...
import { argTypesEqual } from '../utils/functionValidation.js';
...
export function argTypesFromSignature(sig) {
  return (sig?.inputs || []).map(inp => inp.type || 'Number');
}

/**
 * Returns true if loading `sheet` would collide with an already-loaded variant
 * that came from a different source. Same-source reloads are not collisions.
 */
export function detectLoadCollision(sheet, loadedFunctions) {
  const sig = sheet?.publishedVersion?.signature;
  if (!sig) return false;
  const publishedName = sheet.publishedVersion?.publishedName || sheet.name;
  const newArgTypes = argTypesFromSignature(sig);
  return loadedFunctions.some(f => {
    if (f.id === sheet.functionId) return false;
    if (f.name !== publishedName) return false;
    return argTypesEqual(argTypesFromSignature(f.signature), newArgTypes);
  });
}
