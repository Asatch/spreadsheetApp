...
export function createFunctionsDialog() {
  ...
  function initTreeToolbar() {
    ...
    // + New dropdown (re-queried each tree render — rebuild controller)
    newPopoverController?.destroy();
    newPopoverController = null;
    const newBtn = viewContainerEl.querySelector('.fn-new-btn');
    const newMenu = viewContainerEl.querySelector('.fn-new-menu');
    if (newBtn && newMenu) {
      newPopoverController = createPopover({ trigger: newBtn, popover: newMenu });
    }
        ...
        return `
          <div class="tree-item fn-loaded-item" data-function-id="${func.id}">
            ...
            <div class="tree-item-actions">
              ...
              <button type="button" class="tree-btn-icon fn-unload-btn" data-function-id="${func.id}" title="Unload">&times;</button>
            </div>
          </div>
        `;
      }).join('');
  ...
  function renderActionBar() {
    ...
    if (!isMulti && scenario) {
      ...
      if (published) {
        ...
        if (!loaded) {
          const collides = detectLoadCollision(sheet, loadedFunctions);
          const label = collides ? 'Replace existing variant' : 'Load Function';
          const replaceAttr = collides ? ' data-replace="true"' : '';
          buttons += `<button type="button" class="fn-action-btn fn-action-btn-primary fn-action-load" data-function-id="${sheet.functionId}"${replaceAttr}>${label}</button>`;
        }
      }
  ...
  async function fetchExamplesCatalog() {
    ...
    // Optional personal catalog (gitignored, machine-local). 404 = not present.
    try {
      const res = await fetch(`${import.meta.env.BASE_URL}examples/catalog-personal.json`);
      if (res.ok) {
        const personal = await res.json();
        if (Array.isArray(personal)) examplesCatalog = examplesCatalog.concat(personal);
      }
    } catch (_) {
      // ignore — personal catalog is optional
    }
  ...
  async function handleActionBarClick(e) {
    ...
    if (target.classList.contains('fn-action-load')) {
      ...
      if (functionId) await loadSingleFunction(functionId, { replace });
      return;
    }
  ...
  async function loadSingleFunction(functionId, options = {}) {
    try {
      const funcDef = await loadFunctionById(functionId);
      const result = registerFunction({ [funcDef.name]: funcDef }, options);
      if (result && result.success === false) {
        const c = result.conflicts[0];
        alert(`Cannot load: a variant ${formatArgList(c.argTypes)} for ${c.name} is already loaded from another function.`);
        return;
      }
      loadedFunctions = getFunctionsWithMetadata?.() || [];
      missingFunctions = detectMissingFunctions(getNodeCalcData);
  ...
  function formatArgList(argTypes) {
    return argTypes ? `(${argTypes.join(', ')})` : '';
  }
  ...
  async function loadSelectedFunctions() {
    ...
    // Register one funcDef at a time so multiple selected sheets that publish
    // the same name (with different signatures) merge into the entry's variant
    // list rather than collapsing into a single map slot.
    const eligible = sheetsToLoad.filter(s =>
      s.functionId && s.publishedVersion && !isLoaded(s.functionId)
    );
    const loaded = await Promise.all(eligible.map(async sheet => {
      try {
        return { sheet, funcDef: await loadFunctionById(sheet.functionId) };
      } catch (err) {
    ...
    for (const entry of loaded) {
      if (!entry) continue;
      const result = registerFunction({ [entry.funcDef.name]: entry.funcDef });
      if (result && result.success === false) {
    ...
    if (allConflicts.length > 0) {
      const lines = allConflicts.map(c =>
        `${c.name}${formatArgList(c.argTypes)}: collides with already-loaded variant`);
      alert(`Some functions could not be loaded:\n${lines.join('\n')}`);
    }

    if (registered > 0 || allConflicts.length > 0) {
      loadedFunctions = getFunctionsWithMetadata?.() || [];
      missingFunctions = detectMissingFunctions(getNodeCalcData);
      updateLoadedBadge();
    }
  ...
  function handleLoadedViewClick(e) {
    ...
    if (target.classList.contains('fn-unload-btn')) {
      const functionId = target.dataset.functionId;
      if (functionId && unregisterFunction) {
