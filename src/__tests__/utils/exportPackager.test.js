 ...
 * scenariosConfig is { [scenarioId]: { meta, data } }.
 */
function createMocks(sheetsConfig, foldersConfig = {}, scenariosConfig = {}) {
  const storageEngine = {
    ...
    listScenarioEntries: vi.fn(async () => {
      const out = {};
      for (const [id, config] of Object.entries(scenariosConfig)) {
        out[id] = config.meta;
      }
      return out;
    }),
  ...
  const opfsService = {
    ...
    loadScenario: vi.fn(async (id) => {
      const config = scenariosConfig[id];
      if (!config) throw new Error(`No scenario data for ${id}`);
      return config.data;
    }),
  };
...
async function roundTrip(sheetsConfig, foldersConfig, exportOpts = {}) {
  const { scenariosConfig, ...rest } = exportOpts;
  const { storageEngine, opfsService } = createMocks(sheetsConfig, foldersConfig, scenariosConfig);
  const sheetIds = new Set(Object.keys(sheetsConfig).filter(
    id => !rest.excludeFromSelection?.has(id)
  ));
...
describe('export → import round-trip', () => {
  ...
  test('manifest version is 2.1', async () => {
    const importData = await roundTrip({
      'sheet-1': {
        metadata: {
          name: 'S', description: '', type: 'standard', folderId: null,
          createdAt: 'x', updatedAt: 'x', functionId: null, publishedVersion: null,
          hasUnpublishedChanges: false,
        },
        draftXml: buildXml({ name: 'S' }),
        publishedXml: null,
        publishedJs: null,
      },
    });
  ...
  test('scenarios for exported functions are bundled', async () => {
    const xml = buildXml({ name: 'TARGET', functionId: 'func-target' });
    const sheetsConfig = {
      'sheet-1': {
        metadata: {
          name: 'TARGET', description: '', type: 'standard', folderId: null,
    ...
    const scenariosConfig = {
      'scenario-1': {
        ...
        data: {
          inputs: { X: { category: 'decision', values: [1, 2] } },
          results: null,
        },
      },
    };

    const importData = await roundTrip(sheetsConfig, {}, { scenariosConfig });

    expect(importData.scenarios).toBeDefined();
    expect(Object.keys(importData.scenarios)).toEqual(['scenario-1']);
    expect(importData.scenarios['scenario-1'].meta.name).toBe('My sweep');
    expect(importData.scenarios['scenario-1'].data.inputs.X.values).toEqual([1, 2]);
  });

  test('scenarios for non-exported functions are filtered out', async () => {
    const xml = buildXml({ name: 'A', functionId: 'func-a' });
    const sheetsConfig = {
      'sheet-a': {
        metadata: {
          name: 'A', description: '', type: 'standard', folderId: null,
          createdAt: 'x', updatedAt: 'x',
          functionId: 'func-a',
          publishedVersion: { versionId: 'v1', versionString: '1.0', publishedAt: 'x' },
          hasUnpublishedChanges: false,
    ...
    const scenariosConfig = {
      'scenario-irrelevant': {
        meta: {
          name: 'Other',
          functionId: 'func-elsewhere',  // not in this export
          functionName: 'OTHER',
          createdAt: 'x', updatedAt: 'x', folderId: null,
        },
        data: { inputs: {}, results: null },
      },
    };

    const importData = await roundTrip(sheetsConfig, {}, { scenariosConfig });

    expect(importData.scenarios || {}).toEqual({});
    expect(importData.manifest.scenarios).toBeUndefined();
  });
});
