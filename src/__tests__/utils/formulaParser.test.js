...
import { parseFormula as parseTokens } from '../../utils/formulaParser';
import { tokenize, TokenType } from '../../utils/formulaTokenizer';

const parseFormula = (input) => parseTokens(
  typeof input === 'string'
    ? tokenize(input).filter(t => t.type !== TokenType.WHITESPACE)
    : input
);
...
describe('formulaParser', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should reject bare percentage as syntax error', () => {
  ...
  describe('Array Literals', () => {
    it('should parse simple numeric array literal', () => {
      const result = parseFormula('={1,2,3}');
      expect(result.precedents).toEqual(['ARRAY', '1', '2', '3']);
      expect(result.anonymousExpressions).toEqual([]);
    });

    it('should parse array literal with cell references', () => {
      const result = parseFormula('={A1,B1,C1}');
      expect(result.precedents).toEqual(['ARRAY', 'A1', 'B1', 'C1']);
      expect(result.anonymousExpressions).toEqual([]);
    });

    it('should parse array literal with expressions', () => {
      const result = parseFormula('={A1+1,B1*2}');
      expect(result.precedents).toEqual(['ARRAY', '=A1+1', '=B1*2']);
      expect(result.anonymousExpressions).toContain('=A1+1');
      expect(result.anonymousExpressions).toContain('=B1*2');
    });

    it('should parse array literal as function argument', () => {
      const result = parseFormula('=SUM({1,2,3})');
      expect(result.precedents).toEqual(['SUM', '={1,2,3}']);
      expect(result.anonymousExpressions).toContain('={1,2,3}');
    });

    it('should parse array literal mixed with other args', () => {
      const result = parseFormula('=SUM({1,2,3},A1)');
      expect(result.precedents[0]).toBe('SUM');
      expect(result.anonymousExpressions).toContain('={1,2,3}');
    });

    it('should parse array literal with single element', () => {
      const result = parseFormula('={5}');
      expect(result.precedents).toEqual(['ARRAY', '5']);
    });

    it('should parse nested parens inside braces inside function', () => {
      // SUM({(A1+1), (B1*2)}) — parens inside braces inside parens
      const result = parseFormula('=SUM({(A1+1),(B1*2)})');
      expect(result.precedents[0]).toBe('SUM');
      expect(result.anonymousExpressions).toContain('={(A1+1),(B1*2)}');
    });

    it('should treat overlapping brace/paren pairs as syntax error', () => {
      // {(}) — each has a "partner" but they interleave incorrectly
      expect(parseFormula('={(})').precedents).toEqual(['PROCEED', '#SYNTAX!']);
      expect(parseFormula('={1,(2},3)').precedents).toEqual(['PROCEED', '#SYNTAX!']);
      expect(parseFormula('={(1,2})').precedents).toEqual(['PROCEED', '#SYNTAX!']);
    });

    it('should handle unbalanced braces as syntax error', () => {
      const result = parseFormula('={1,2,3');
      expect(result.precedents).toEqual(['PROCEED', '#SYNTAX!']);
    });
  });
