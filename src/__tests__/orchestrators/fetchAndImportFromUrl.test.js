...
describe('fetchAndImportFromUrl', () => {
  ...
  it('replaces existing folder when packageId already downloaded', async () => {
    // Updated behavior: re-import deletes the existing copy and imports fresh,
    // so users picking up an updated zip get the new contents (not stale).
    setupHappyPath();
    const deleteFolder = vi.fn().mockResolvedValue();
    const deps = createMockDeps({
    ...
    // Existing folder was deleted before re-import
    expect(deleteFolder).toHaveBeenCalledWith('existing-folder');
    // Then a fresh import ran
    expect(executeImport).toHaveBeenCalledTimes(1);
    expect(publishImportedSheets).toHaveBeenCalledTimes(1);
    expect(result).toEqual({
      entrySheetId: 'local-new-id',
