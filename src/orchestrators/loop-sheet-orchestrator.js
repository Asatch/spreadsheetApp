...
import { adjustTokensByOffset, collapseTokensToRow1 } from '../utils/clipboardUtils.js';
import { tokenize } from '../utils/formulaTokenizer.js';
...
export function createLoopSheetOrchestrator(config = {}) {
  ...
  /**
   * Snapshot row-1 templates once — tokenize each formula, remember non-formulas
   * as strings. The caller iterates many rows against this snapshot, cloning +
   * shifting tokens per row instead of re-tokenizing.
   * @returns {{formulas: Array<[string, import('../utils/formulaTokenizer.js').Token[]]>, plain: Array<[string, string]>}}
   */
  function snapshotRow1Templates() {
    const formulas = [];
    const plain = [];
    for (const key of getRow1CellKeys()) {
      const parsed = parseCellKey(key);
      if (!parsed) continue;
      const value = canonicalValuesEngine.getValue(key);
      if (value && value.startsWith('=')) {
        formulas.push([key, tokenize(value)]);
      } else {
        plain.push([key, value]);
      }
    }
    return { formulas, plain };
  }

  /**
   * Apply a single iteration row against the pre-tokenized row-1 snapshot.
   * Clones tokens per formula and shifts by the row offset in place.
   */
  function applyGeneratedRow(targetRow, templates) {
    const offset = targetRow - 1;
    const entries = [];
    for (const [srcKey, tokens] of templates.formulas) {
      const { col } = parseCellKey(srcKey);
      const cloned = tokens.map(t => ({ ...t }));
      adjustTokensByOffset(cloned, offset, 0);
      entries.push([`${col}${targetRow}`, null, cloned]);
    }
    for (const [srcKey, value] of templates.plain) {
      const { col } = parseCellKey(srcKey);
      entries.push([`${col}${targetRow}`, value]);
    }
    if (entries.length > 0) {
  ...
  function flushPendingRefreshes() {
    ...
    if (typeof document === 'undefined') { pendingRefreshCells.clear(); return; }
    const updates = [];
  ...
  function runIterationLoop() {
    ...
    // Generate rows until stop condition. Tokenize row-1 templates once; each
    // iteration clones and shifts tokens rather than re-tokenizing formulas.
    const templates = snapshotRow1Templates();
    let currentRow = 2;
    while (currentRow < MAX_ITERATIONS) {
      ...
      applyGeneratedRow(currentRow, templates);
   ...
   */
  function setValue(cellKey, rawValue, tokens) {
    if (!isCellEditable(cellKey)) {
      return false;
    }
    // Token path: formula-bar commit hands us pre-tokenized input. Collapse on
    // tokens and hand to CVE classified, skipping a re-tokenize pass.
    // Stop-row entries aren't formulas, so normalizeStopEntries never needs to
    // run on token-path commits of non-stop cells.
    if (tokens && cellKey !== '_STOP0' && cellKey !== '_STOP1') {
      const collapsed = collapseTokensToRow1(tokens.map(t => ({ ...t })));
      canonicalValuesEngine.setClassified(cellKey, collapsed);
      return true;
    }
    // Programmatic caller without tokens, or a stop-row entry. Stop rows aren't
    // formulas so nothing to collapse; non-stop formulas tokenize here and feed
    // the classified path to avoid a second tokenize inside CVE.
    const isStop = cellKey === '_STOP0' || cellKey === '_STOP1';
    if (!isStop && rawValue && rawValue.startsWith('=')) {
      const collapsed = collapseTokensToRow1(tokenize(rawValue));
      canonicalValuesEngine.setClassified(cellKey, collapsed);
      return true;
    }
    const normalized = normalizeStopEntries([[cellKey, rawValue]]);
   ...
   */
  /**
   * Editability-aware batch setter. Accepts either `[key, value]` or
   * `[key, value, tokens]` entries — matches CVE's setBatch shape. Formula
   * entries (either pre-tokenized or detected from the string) are collapsed
   * to row 1; stop-row entries pass through normalizeStopEntries. Single CVE
   * call → single history checkpoint.
   * @param {Array<[string, string|null, import('../utils/formulaTokenizer.js').Token[]?]>} entries
   * @returns {boolean}
   */
  function setBatch(entries) {
    const editable = entries.filter(([key]) => isCellEditable(key));
    if (editable.length === 0) return false;

    const plainEntries = [];
    const formulaEntries = [];
    for (const entry of editable) {
      const [key, value, tokens] = entry;
      const isStop = key === '_STOP0' || key === '_STOP1';
      if (tokens && !isStop) {
        const collapsed = collapseTokensToRow1(tokens.map(t => ({ ...t })));
        formulaEntries.push([key, null, collapsed]);
      } else if (!isStop && value && typeof value === 'string' && value.startsWith('=')) {
        const collapsed = collapseTokensToRow1(tokenize(value));
        formulaEntries.push([key, null, collapsed]);
      } else {
        plainEntries.push([key, value]);
      }
    }
    const normalized = plainEntries.length > 0 ? normalizeStopEntries(plainEntries) : [];
    const merged = [...formulaEntries, ...normalized];
    if (merged.length === 0) return false;
    canonicalValuesEngine.setBatch(merged);
    return true;
  }
  ...
  function initializeAll() {
    ...
    const components = config.headless
      ...
      : { ...config.persistentComponents, grid: createGrid(applyLoopBounds({ maxCol: 'H', maxRow: 29 })) };
    ...
    // Headless mode: force scratchpad so auto-save paths bail.
    if (config.headless) {
  ...
  return {
    ...
    runIteration() {
      runIterationLoop();
      return { iterationCount, hitMax: iterationCount >= MAX_ITERATIONS };
    },
    ...
    /**
     * Headless entry point: load a loop spreadsheet from an XML string.
     * After load, the afterLoad hook runs the iteration loop.
     */
    ...
    /**
     * Returns [{ key, value }] for each output column. For output_mode="all"
     * outputs (column letter as key), value is an array across iterations.
     */
    getOutputs() {
      const cells = panels.getOutputCells();
      const modes = panels.getOutputModes() || {};
      return cells.map(key => {
        if (modes[key] === 'all') {
          const values = [];
          for (let row = 0; row <= iterationCount; row++) {
            const cellKey = `${key}${row}`;
            const v = calculationEngine.getCellValue(cellKey);
            if (v !== null && v !== undefined && v !== '') values.push(v);
          }
          return { key, value: values };
        }
        const cellKey = `${key}${iterationCount}`;
        return { key, value: calculationEngine.getCellValue(cellKey) };
      });
    },
