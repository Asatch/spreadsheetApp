 ...
 *   - namedRanges: Array (currently empty)
...
export function parseXml(xmlString) {
  ...
  const namedInputNames = new Set(namedInputsList.map(n => n.name));

  // Derive named ranges: non-cell-reference keys that aren't inputs are named ranges.
  // Their canonical value (=notation) tells us what cell(s) they point to.
  const namedRangesList = [];
  for (const [key, canonical] of canonicalValues) {
    if (!isCellReference(key) && !namedInputNames.has(key) && canonical.startsWith('=')) {
      namedRangesList.push([key, canonical.substring(1)]);
    }
  }
