...
function emitPersistedNodes(ctx, G, sortedNodes, conversionRules) {
  ...
  for (const nodeId of sortedNodes) {
    ...
    if (nodeAttrs.persist) {
      console.log(`[emitPersistedNodes] node ${nodeId}, func=${nodeAttrs.function_name || nodeAttrs.node_type}`);
      if (usesForcePersistedTemplate(G, nodeId, conversionRules)) {
        const statement = codeNode(ctx, G, nodeId, true, conversionRules);
        console.log(`[emitPersistedNodes] node ${nodeId} codeNode done, len=${statement?.length}`);
        ...
        console.log(`[emitPersistedNodes] node ${nodeId} codeNode done, len=${expr?.length}`);
        drainPendingStatements(ctx, bodyStatements);
  ...
  console.log('[emitPersistedNodes] final drain, pending:', ctx.pendingStatements?.length, 'body:', bodyStatements.length);
  drainPendingStatements(ctx, bodyStatements);
  console.log('[emitPersistedNodes] done, total statements:', bodyStatements.length, 'total chars:', bodyStatements.reduce((s, st) => s + st.length, 0));
...
function convertAndTranspile(G, conversionRules, lang) {
  console.log('[convertAndTranspile] start, nodes:', G.nodeIds().length);
  ...
  if (G.graph.iteration_body_dag) {
    console.log('[convertAndTranspile] transpiling inner DAG, nodes:', G.graph.iteration_body_dag.nodeIds().length);
    const innerDag = G.graph.iteration_body_dag;
    innerFuncCode = transpileInnerDag(ctx, innerDag, conversionRules);
    console.log('[convertAndTranspile] inner DAG done');
    ctx.usedFunctions = new Set();
  }

  console.log('[convertAndTranspile] markNodesToPersist');
  dags.markNodesToPersist({
    G,
    conversionRules,
    allOutputs: false,
  ...
  console.log('[convertAndTranspile] emitPersistedNodes');
  const bodyStatements = emitPersistedNodes(ctx, G, sortedNodes, conversionRules);

  console.log('[convertAndTranspile] building return statement');
  let returnStatement;
  let returnType;
  ...
  console.log('[convertAndTranspile] assembleFunction');
...
export function transpileDags(settings, lang = JAVASCRIPT_SYNTAX) {
  ...
  console.log('[transpileDags] phase: convertGraph');
  dags.convertGraph({
    dagToConvert: G,
    conversionRules,
    signatureDefinitionLibrary,
  ...
  console.log('[transpileDags] phase: convertAndTranspile');
  let code = convertAndTranspile(G, conversionRules, lang);

  if (loopFunctions && Object.keys(loopFunctions).length > 0) {
    console.log('[transpileDags] phase: generateLoopHelperFunctions');
  ...
  if (Object.keys(overrides).length > 0) {
    console.log('[transpileDags] phase: collectUsedOverrides');
  ...
  console.log('[transpileDags] phase: wrapModule');
