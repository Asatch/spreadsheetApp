...
import { exportAsHtml } from '../../utils/htmlExporter.js';
...
export function openDrilldown({ functionId, versionId, args, sheetType }) {
  ...
  const basePath = sheetType === 'loop' ? import.meta.env.BASE_URL + 'loop.html' : import.meta.env.BASE_URL;
  const url = `${window.location.origin}${basePath}?${params.toString()}`;

  window.open(url, '_blank');
  console.log(`[OrchestrationCore] Drill-down to ${functionId || versionId} (${sheetType || 'standard'})`);
}
...
export function createOrchestrationCore() {
  ...
  function init(initConfig) {
    ...
    console.log(`${deps.config.logPrefix} Core initialized`);
  }
  ...
  function recordScenarioOutputs(scenariosToProcess, inputNames, outputNames) {
    ...
    if (scenariosToProcess.length === 0 || inputNames.length === 0 || outputNames.length === 0) {
      console.log(`${logPrefix} No scenarios or no inputs/outputs defined, skipping output generation`);
    ...
    console.log(`${logPrefix} Saved original inputs:`, originalInputs);
    ...
    for (let i = 0; i < scenariosToProcess.length; i++) {
      ...
      if (!hasAllInputs) {
        console.log(`${logPrefix} Skipping scenario ${i + 1} - missing inputs`);
        continue;
      }

      console.log(`${logPrefix} Processing scenario ${i + 1}/${scenariosToProcess.length}...`);
      ...
      console.log(`${logPrefix} Scenario ${i + 1} outputs:`, outputs);
      scenariosToProcess[i].outputs = outputs;
    }

    console.log(`${logPrefix} Restoring original inputs...`);
    ...
    console.log(`${logPrefix} Scenario output recording complete`);
    return scenariosToProcess;
  }
  ...
  function handleClearCells() {
    ...
    console.log(`${deps.config.logPrefix} Clearing ${cells.length} cells`);
  ...
  function handleUndo() {
    ...
    if (success) {
      console.log(`${deps.config.logPrefix} Undo - state restored`);
  ...
  function handleRedo() {
    ...
    if (success) {
      console.log(`${deps.config.logPrefix} Redo - state restored`);
      deps.clipboardEngine.cancelCut();
  ...
  function handleApplyCellFormat(format) {
    ...
    console.log(`${logPrefix} Applied ${typeKey} format to ${cells.length} cells`);
  }
  ...
  function handleApplyDefaultFormat(format) {
    ...
    console.log(`${logPrefix} Updated default ${typeKey} format`);
  }
  ...
  async function applySpreadsheetState(spreadsheet, options = {}) {
    ...
    if (spreadsheet.customFunctionIds && spreadsheet.customFunctionIds.length > 0) {
      console.log(`${logPrefix} Loading custom functions:`, spreadsheet.customFunctionIds);
      try {
        ...
        for (const [funcId, result] of results) {
          if (result.error) {
            console.warn(`${logPrefix} Failed to load function ${funcId}:`, result.error);
          } else {
            const consumerName = nameMap[funcId] || result.name;
            functionsToRegister[consumerName] = result;
          }
        }
        if (Object.keys(functionsToRegister).length > 0) {
          deps.calculationEngine.registerFunction(functionsToRegister);
          // Update singleArrayFunctions so formula normalization works
          // for custom functions after restore (mirrors orchestrator-shared logic)
          for (const [name, funcDef] of Object.entries(functionsToRegister)) {
            const isSingleArray = funcDef.variants?.every(v =>
              v.argTypes.length === 1 && v.argTypes[0].startsWith('ARRAY[')
            );
            if (isSingleArray) {
              deps.canonicalValuesEngine.updateSingleArrayFunctions(name, true);
            }
          }
        }
      } catch (error) {
    ...
    if (argValues) {
      ...
      for (let i = 0; i < namedInputs.length && i < argValues.length; i++) {
        ...
        console.log(`${logPrefix} Setting input ${inputName} = ${value}`);
    ...
    if (testCases.length > 0) {
      console.log(`${logPrefix} Loaded ${testCases.length} test cases`);
    }
  ...
  async function loadDrilldown({ functionId, versionId, argValues }) {
    const logPrefix = deps.config.logPrefix;
    console.log(`${logPrefix} Loading drill-down spreadsheet for:`, versionId || functionId);
    ...
    if (versionId && funcDef.versionId && versionId !== funcDef.versionId) {
      console.log(`${logPrefix} Version mismatch: requested ${versionId}, showing ${funcDef.versionId}`);
    ...
    console.log(`${logPrefix} Parsed spreadsheet:`, {
      name: spreadsheet.name,
      functionName: resolvedFunctionName,
      versionId: resolvedVersionId,
      versionString: resolvedVersionString,
      sourceSheetId,
      outputCells: spreadsheet.outputCells,
      namedInputs: spreadsheet.namedInputs,
      customFunctionIds: spreadsheet.customFunctionIds,
      testCases: spreadsheet.testCases?.length,
      argValues
    });
    ...
    // Check if we have the source sheet locally for merge option
    const sheetManifest = await deps.storageEngine.getSheetManifest();
    const hasLocalSource = sourceSheetId && !!sheetManifest.sheets[sourceSheetId];
    ...
    console.log(`${logPrefix} Drill-down spreadsheet loaded in scratchpad mode`);
  ...
  async function loadFromOpfs(id) {
    const logPrefix = deps.config.logPrefix;
    console.log(`${logPrefix} Loading spreadsheet from OPFS:`, id);

    const { xml, metadata } = await deps.storageEngine.loadSpreadsheetFromOpfs(id);
    const spreadsheet = parseXml(xml);

    console.log(`${logPrefix} Loaded spreadsheet:`, {
      name: metadata.name,
      type: metadata.type,
      cells: spreadsheet.canonicalValues?.length
    });
    ...
    console.log(`${logPrefix} Spreadsheet loaded successfully`);
  ...
  async function publishLocal() {
    ...
    try {
      ...
      console.log(`${logPrefix} Published locally: ${functionName} (${functionId})`);
    ...
    return {
      onPreviewMerge: async () => {
        ...
        try {
          ...
          window.location.href = `${window.location.origin}${basePath}?id=${previewInfo.basedOnSpreadsheetId}`;
      ...
      onPreviewDiscard: async () => {
        ...
        try {
          ...
          if (previewInfo?.versionId || previewInfo?.functionId) {
            ...
            window.location.href = `${window.location.origin}${basePath}?${params.toString()}`;
          } else {
      ...
      onPreviewFork: async (newName) => {
        try {
          if (deps.storageEngine.isScratchpadMode()) {
            ...
            console.log(`${logPrefix} Scratchpad forked to new sheet:`, newName, newId);
  ...
  function getSharedCallbacks() {
    ...
    return {
      ...
      // Export current sheet as portable HTML
      onExportHtml: async () => {
        const currentId = deps.storageEngine.getCurrentSpreadsheetId();
        if (!currentId) return;
      ...
      onDeleteCurrent: async () => {
        ...
        try {
          ...
          window.location.href = window.location.origin + deps.config.basePath;
        } catch (error) {
      ...
      onOpenSpreadsheet: async (id) => {
        ...
        if (deps.hooks.onOpenSpreadsheet) {
          ...
          const base = sheetType === 'loop' ? import.meta.env.BASE_URL + 'loop.html' : import.meta.env.BASE_URL;
          window.location.href = `${window.location.origin}${base}?id=${id}`;
        }
      },
      ...
      onZipDrop: async (file) => {
        const result = await deps.importDialog.open(file);
        if (result) {
          console.log(`${logPrefix} Import complete:`, result);
        }
      },
      onXmlDrop: async (file) => {
        try {
          const { id, type } = await deps.storageEngine.importFile(file);
          const base = type === 'loop' ? '/loop.html' : '/index.html';
      ...
      onHtmlDrop: async (file) => {
        try {
          ...
          const result = await deps.importDialog.open(zipFile);
          if (result) {
            console.log(`${logPrefix} HTML import complete:`, result);
          }
        } catch (error) {
      ...
      onFolderDrop: async (directoryEntry) => {
        try {
          ...
          const result = await deps.importDialog.openWithData(data, directoryEntry.name);
          if (result) {
            console.log(`${logPrefix} Folder import complete:`, result);
          }
        } catch (error) {
      ...
      onFolderPick: async (fileList) => {
        try {
          ...
          const result = await deps.importDialog.openWithData(data, folderName);
          if (result) {
            console.log(`${logPrefix} Folder import complete:`, result);
          }
        } catch (error) {
