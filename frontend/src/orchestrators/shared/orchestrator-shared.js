...
import { createCanonicalValuesEngine } from '../../Engines/canonicalValuesEngine.js';
...
export function createEngines() {
  return {
    ...
    calculationEngine: createCalculationEngine(),
  ...
  return {
    ...
    functionsDialog: {
      ...
      onNewLoop: { type: 'function', value: (folderId) => {
        const url = new URL(import.meta.env.BASE_URL + 'loop.html', window.location.origin);
        if (folderId) url.searchParams.set('folder', folderId);
        window.location.href = url.toString();
      }},
      ...
      registerFunction: { type: 'function', value: (functions) => {
        ...
        calculationEngine.registerFunction(functions);
        storageEngine.markDirty();
      }},
      getFunctionsWithMetadata: { type: 'function', value: () => calculationEngine.getFunctionsWithMetadata() },
      unregisterFunction: { type: 'function', value: (name) => {
        canonicalValuesEngine.updateSingleArrayFunctions(name, false);
        const dependents = Array.from(calculationEngine.getDependentsOf(name));
        calculationEngine.unregisterFunction(name);
        if (dependents.length > 0) {
          canonicalValuesEngine.renormalizeFormulas(dependents);
        }
        storageEngine.markDirty();
  ...
  return {
    ...
    onDrilldown: { type: 'function', value: (cellKey) => {
      ...
      if (drilldownInfo) {
        ...
        console.log('[Orchestrator] No custom function drill-down available for', cellKey);
      }
    }},
...
export async function fetchAndImportFromUrl(url, { storageEngine, opfsService, functionCompiler }) {
  ...
  // Check for existing download by packageId
  const packageId = importData.manifest?.packageId || null;
  if (packageId) {
    const existing = await storageEngine.findFolderByPackageId(packageId);
    if (existing) {
      return {
        entrySheetId: existing.folder.entrySheetId || null,
        entrySheetType: existing.folder.entrySheetType || 'standard',
        importFolderId: existing.folderId,
