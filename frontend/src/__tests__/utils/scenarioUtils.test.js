...
describe('findTopDriver', () => {
  ...
  test('warns and skips when baseline is not in run data', () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    // B baseline is 99 which doesn't appear in any run
    const runs = [
      { inputs: { A: 1, B: 10 }, outputs: { out: 50 } },
      { inputs: { A: 2, B: 10 }, outputs: { out: 60 } },
      { inputs: { A: 1, B: 20 }, outputs: { out: 70 } },
      { inputs: { A: 2, B: 20 }, outputs: { out: 80 } },
    ];
    const inputConfigs = {
      A: { values: [1, 2], baseline: 1 },
      B: { values: [10, 20], baseline: 99 },  // baseline not in runs
    };
    // A's baseline runs require B=99, which doesn't exist → warn for A
    // B's baseline runs require A=1, which exists → B can still compute
    const result = findTopDriver(runs, 'out', ['A', 'B'], inputConfigs);
