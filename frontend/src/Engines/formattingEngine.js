...
import { TYPE_HIERARCHY, TypeService, isArrayType } from '../utils/typeService.js';
import {
  formatDate,
  formatDateTime,
  formatTime,
...
export function createFormattingEngine() {
  ...
  function refreshCellWithUpdatedStyles(cellKey) {
    ...
    if (display) {
      ...
      const semanticStyles = {
        ...
        alignment: userStyles.alignment || (node ? getDefaultAlignment(node.type) : 'left')
      };
  ...
  function computeDisplayValue(node, cellKey) {
    ...
    const semanticStyles = {
      ...
      // Only apply default alignment if user hasn't explicitly set one
      alignment: userStyles.alignment || getDefaultAlignment(node.type)
    };
   ...
   */
  function applyBold() {
    console.log('[FormattingEngine] Toggle bold');
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Toggled bold for ${cells.length} cells (${allBold ? 'removed' : 'applied'})`);
  }
  ...
  function applyItalic() {
    console.log('[FormattingEngine] Toggle italic');
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Toggled italic for ${cells.length} cells (${allItalic ? 'removed' : 'applied'})`);
  }
  ...
  function increaseFontSize() {
    console.log('[FormattingEngine] Increase font size');
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Increased font size for ${cells.length} cells`);
  }

  function decreaseFontSize() {
    console.log('[FormattingEngine] Decrease font size');
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Decreased font size for ${cells.length} cells`);
  }
   ...
   */
  function alignLeft() {
    console.log('[FormattingEngine] Align left');
    applyAlignment('left');
  }
  ...
  function alignCenter() {
    console.log('[FormattingEngine] Align center');
   ...
   */
  function alignRight() {
    console.log('[FormattingEngine] Align right');
  ...
  function applyAlignment(alignment) {
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Applied ${alignment} alignment to ${cells.length} cells`);
  }
  ...
  function clearFormatting() {
    console.log('[FormattingEngine] Clear formatting');
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Cleared formatting for ${cells.length} cells`);
  }
  ...
  function applyFormatRules(cellKeys, formatSettings) {
    console.log(`[FormattingEngine] Applying format rules to ${cellKeys.length} cells`);
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Applied ${typeKey} format rules to ${cellKeys.length} cells`);
    return { success: true };
  }
  ...
  function updateSpreadsheetDefault(formatType, formatSettings) {
    console.log(`[FormattingEngine] Updating spreadsheet default for ${formatType}`, formatSettings);
    ...
    console.log(`[FormattingEngine] Updated spreadsheet default for ${formatType}`);
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Refreshed ${refreshedCount} cells using ${formatType} default`);
  ...
  function setFormattingBatch(updates) {
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Set formatting for ${updates.length} cells`);
  }
  ...
  function clearFormattingBatch(cellKeys) {
    ...
    onFormattingChange?.();
    console.log(`[FormattingEngine] Cleared formatting for ${cellKeys.length} cells`);
  }
  ...
  return {
    ...
    init(deps) {
      ...
      console.log('[FormattingEngine] Initialized');
    },
    ...
    restoreSnapshot(data) {
      console.log('[FormattingEngine] Restoring snapshot...');
      ...
      console.log('[FormattingEngine] Restored:', {
        formatRules: formatRules.size,
        cellStyles: cellStyles.size,
        defaults: Object.keys(spreadsheetDefaults).length
      });
    },
