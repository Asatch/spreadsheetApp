  ...
  function processInputs(changedInfo) {
    ...
    for (const [key, info] of changedInfo) {
      ...
      if (type === 'formula') {
        ...
        computeDisplayValue({ refValue: '', type: 'Text' }, key);
  ...
  if (dependents) {
    ...
    if (dependents.size === 0) {
      ...
      if (precedentRef.startsWith('=')) {
        ...
        // Notify CanonicalValuesEngine to GC the anonymous expression from its storage
        if (onDeleteAnonymous) {
...
function processPhase3() {
  ...
  while (readyQueue.length > 0) {
    ...
    // Add display formatting
    computeDisplayValue(node, cellRef);
  ...
  if (evaluationCells.size > 0) {
    for (const nodeRef of evaluationCells) {
      ...
      computeDisplayValue(node, nodeRef);
    }
  }
...
function queueDependents(nodeRef) {
  ...
  return {
     ...
     *
     * @example
     * calcEngine.registerFunction({ SUM: { variants: [...] }, ADD: { variants: [...] } });
     */
    registerFunction(functions) {
      ...
      processPhase2();
      console.log(`[CalcEngine] Registered ${count} function(s)`);
     ...
     *
     * Removes a function from nodeCalcData. Formulas using it will
     * get #NAME! error on next evaluation.
     *
     * @param {string} name - Function name to unregister
     * @returns {boolean} True if function was removed
     */
    unregisterFunction(name) {
      const node = nodeCalcData.get(name);

      if (node?.type !== 'function') {
        return false;
      }

      nodeCalcData.delete(name);
      console.log(`[CalcEngine] Unregistered function: ${name}`);

      // Re-evaluate formulas that were using this function (they'll get #NAME!)
      queueDependents(name);
      processPhase2();
     ...
     *
     * Returns info for functions that have metadata (id, version, etc on funcDef).
     *
    ...
    getFunctionsWithMetadata() {
      ...
      for (const [name, node] of nodeCalcData) {
        if (node.type === 'function' && node.refValue?.id) {
          const def = node.refValue;
          result.push({
            name,                          // consumer name (registration key)
            canonicalName: def.name,       // source's published name
            id: def.id,
            versionId: def.versionId,
            version: def.version,
            sheetType: def.sheetType,
    ...
    getDrilldownInfo(cellKey) {
      ...
      while (stack.length > 0) {
        ...
        for (const p of n.precedents) {
          ...
          if (pNode?.type === 'function' && pNode.refValue?.id) {
            funcName = p;
            funcNode = pNode;
            break;
          }
      ...
      const functionId = funcNode.refValue.id;
      ...
      if (callerNode?.precedents?.[0] === funcName) {
        for (let i = 1; i < callerNode.precedents.length; i++) {
          ...
          if (isLiteral(arg)) {
            const converted = convertLiteral(arg);
            argValues.push(TypeService.serializeForInput(converted.refValue, converted.type));
          } else {
            const argNode = nodeCalcData.get(arg);
            if (argNode) {
              argValues.push(TypeService.serializeForInput(argNode.refValue, argNode.type));
            } else {
              argValues.push('');
            }
          }
      ...
      const versionId = funcNode.refValue.versionId || null;
      const functionName = funcNode.refValue.name || null;
      const sheetType = funcNode.refValue.sheetType || 'standard';
      return { functionId, versionId, functionName, argValues, sheetType };
    },
