...
export function createHistoryEngine() {
  ...
  function registerMap(mapName, mapInstance) {
    ...
    console.log(`[HistoryEngine] Registered map: ${mapName}`);
  }
  ...
  function registerRebuildCallback(mapName, callback) {
    ...
    console.log(`[HistoryEngine] Registered rebuild callback for: ${mapName}`);
  }
  ...
  function registerSnapshotProvider(name, getter, restorer) {
    ...
    console.log(`[HistoryEngine] Registered snapshot provider: ${name}`);
  }
  ...
  function undo() {
    if (undoStack.length === 0) {
      console.log('[HistoryEngine] Nothing to undo');
      return false;
    }

    console.log('[HistoryEngine] Undo');
    isRestoring = true;
  ...
  function redo() {
    if (redoStack.length === 0) {
      console.log('[HistoryEngine] Nothing to redo');
      return false;
    }

    console.log('[HistoryEngine] Redo');
    isRestoring = true;
  ...
  return {
    init(deps) {
      ...
      maxSize = maxSize || 50;
      console.log('[HistoryEngine] Initialized with max size:', maxSize);
    },
