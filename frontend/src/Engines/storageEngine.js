...
export function createStorageEngine() {
  ...
  async function publishSheet(sheetId, xml, js, versionString, versionId, functionId, options = {}) {
    ...
    console.log(`[StorageEngine] Published sheet: ${sheetId} as ${functionId} v${versionString}`);
  }
  ...
  function open(options = {}) {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const base = type === 'loop' ? import.meta.env.BASE_URL + 'loop.html' : import.meta.env.BASE_URL + 'index.html';
  ...
  async function createSpreadsheet(name, type = 'standard', options = {}) {
    ...
    console.log(`[StorageEngine] Created spreadsheet: ${id} (${name})`);
    return id;
  }
  ...
  async function createSpreadsheetWithId(id, options) {
    ...
    console.log(`[StorageEngine] Created spreadsheet with ID: ${id} (${name})`);
    return id;
  }
  ...
  async function createSpreadsheetBatch(entries) {
    ...
    await writeSheetManifest(m);
    console.log(`[StorageEngine] Batch created ${entries.length} spreadsheets`);
  }
  ...
  async function deleteSpreadsheet(id) {
    ...
    console.log(`[StorageEngine] Deleted spreadsheet: ${id}`);
  }
  ...
  async function deleteSpreadsheetBatch(ids) {
    ...
    console.log(`[StorageEngine] Batch deleted ${ids.length} spreadsheets`);
  }
  ...
  async function createDraftFromPublished(sheetId) {
    ...
    console.log(`[StorageEngine] Created draft from published for sheet: ${sheetId}`);
  }
  ...
  async function deleteFunction(functionId) {
    ...
    console.log(`[StorageEngine] Deleted function: ${functionId}`);
  }
  ...
  async function createFolder(name, parentId = null, options = {}) {
    ...
    await writeSheetManifest(m);
    console.log(`[StorageEngine] Created folder: ${name} (${folderId})`);
    return folderId;
  }
  ...
  async function deleteFolder(folderId) {
    ...
    // Delete all OPFS sheet files in parallel
    await Promise.all(sheetIds.map(id => opfsService.deleteSheet(id)));

    // Single manifest update for all sheets and folders
  ...
  async function duplicateSpreadsheet(sourceId, newName, targetFolderId = null) {
    ...
    console.log(`[StorageEngine] Duplicated sheet ${sourceId} -> ${newId} (${newName})`);
    return newId;
  }
  ...
  async function findFolderByPackageId(packageId) {
    ...
    const m = await getSheetManifest();
    const downloadsFolderId = Object.entries(m.folders)
      .find(([, f]) => f.name === DOWNLOADS_FOLDER_NAME && !f.parentId)?.[0];
    if (!downloadsFolderId) return null;

    const match = Object.entries(m.folders)
      .find(([, f]) => f.parentId === downloadsFolderId && f.packageId === packageId);
  ...
  return {
    init(deps) {
      ...
      console.log('[StorageEngine] Initialized', opfsService ? 'with OPFS' : 'without OPFS');
    },
    ...
    async clearAllData() {
      ...
      sheetManifest = null;
      currentSpreadsheetId = null;
      console.log('[StorageEngine] All data cleared, caches reset');
    },
    ...
    setOpfsService(service) {
      opfsService = service;
      sheetManifest = null;
      console.log('[StorageEngine] OPFS service set');
    },
  };
}
