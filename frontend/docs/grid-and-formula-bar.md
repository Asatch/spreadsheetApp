...
| `blur` | `handleBlur` | Input loses focus | **If formula mode + losing to gridcell:** saves picking session (no commit). **Otherwise:** commits value. |

### 6.2 Formula Bar `handleKeyDown` Detail

| Key | Condition | Action |
|-----|-----------|--------|
| `ArrowUp` | formula mode | `focusActiveCell()` + `stepSelectionAnchor('up')` -- enters reference picking |
| `ArrowDown` | formula mode | `focusActiveCell()` + `stepSelectionAnchor('down')` -- enters reference picking |
| `ArrowRight` | formula mode, at text end, no selection | `focusActiveCell()` + `stepSelectionAnchor('right')` -- enters reference picking |
| `ArrowLeft` | formula mode, value is just `=`, no selection | `focusActiveCell()` + `stepSelectionAnchor('left')` -- enters reference picking |
| `ArrowUp` | normal mode | `exitEditingAndMove('up')` |
| `ArrowDown` | normal mode | `exitEditingAndMove('down')` |
| `ArrowRight` | normal mode, at text end, no selection | `exitEditingAndMove('right')` |
| `ArrowLeft` | normal mode, at text start, no selection | `exitEditingAndMove('left')` |
| `ArrowLeft/Right` | cursor not at edge, or has selection | Default browser behavior (cursor movement in text) |

**ArrowLeft asymmetry in formula mode:** ArrowRight enters picking at the end of text, but ArrowLeft only enters picking when the value is exactly `=`. In a formula like `=SUM(A1)+B2`, pressing Left at position 0 is almost always cursor navigation, not "pick a reference to the left." The only case where Left-into-picking is useful is when you've just typed `=` and want to start picking immediately. ArrowRight doesn't have this problem because being at the end of a formula usually means extending it.
...
2. User presses End to deselect, cursor at end of "=SUM("
   ...
   → formulaEditingMode=true, ArrowDown always triggers picking
